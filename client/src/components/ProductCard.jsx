...
const ProductCard = ({ id, productName, regularPrice, salePrice, img }) => {
    ...
    return (
        <div className="card product-card position-relative p-3" >
            <Link to={`/product/${id}`} className="product-link d-block position-relative overflow-hidden ">
                ...
                <img className="object-fit-cover w-100 h-100" src={img || "https://cdn.prod.website-files.com/63cffb7c16ab33a28e9734f2/63d4f266958421bafe26c37c_product-05-thumb-p-500.webp"} alt="product" />
            </Link>
            <div className="container-fluid p-3 pb-0">
                <div className="row">
                    <div className="col-8">
                        <h2 className="card-heading font-color text-uppercase fs-4">{productName}</h2>
                        <Link onClick={addToBag} className="btn add-cart-btn fw-medium p-1 px-3">
